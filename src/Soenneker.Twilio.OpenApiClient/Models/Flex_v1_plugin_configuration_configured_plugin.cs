// <auto-generated/>
#pragma warning disable CS0618
using Microsoft.Kiota.Abstractions.Extensions;
using Microsoft.Kiota.Abstractions.Serialization;
using System.Collections.Generic;
using System.IO;
using System;
namespace Soenneker.Twilio.OpenApiClient.Models
{
    [global::System.CodeDom.Compiler.GeneratedCode("Kiota", "1.0.0")]
    #pragma warning disable CS1591
    public partial class Flex_v1_plugin_configuration_configured_plugin : IAdditionalDataHolder, IParsable
    #pragma warning restore CS1591
    {
        /// <summary>The SID of the [Account](https://www.twilio.com/docs/iam/api/account) that the Flex Plugin resource is installed for.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? AccountSid { get; set; }
#nullable restore
#else
        public string AccountSid { get; set; }
#endif
        /// <summary>Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.</summary>
        public IDictionary<string, object> AdditionalData { get; set; }
        /// <summary>A changelog that describes the changes this Flex Plugin Version brings.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? Changelog { get; set; }
#nullable restore
#else
        public string Changelog { get; set; }
#endif
        /// <summary>The SID of the Flex Plugin Configuration that this Flex Plugin belongs to.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? ConfigurationSid { get; set; }
#nullable restore
#else
        public string ConfigurationSid { get; set; }
#endif
        /// <summary>The date and time in GMT when the Flex Plugin was installed specified in [ISO 8601](https://en.wikipedia.org/wiki/ISO_8601) format.</summary>
        public DateTimeOffset? DateCreated { get; set; }
        /// <summary>A descriptive string that you create to describe the plugin resource. It can be up to 500 characters long</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? Description { get; set; }
#nullable restore
#else
        public string Description { get; set; }
#endif
        /// <summary>The friendly name of this Flex Plugin resource.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? FriendlyName { get; set; }
#nullable restore
#else
        public string FriendlyName { get; set; }
#endif
        /// <summary>The phase this Flex Plugin would initialize at runtime.</summary>
        public int? Phase { get; set; }
        /// <summary>Whether the Flex Plugin is archived. The default value is false.</summary>
        public bool? PluginArchived { get; set; }
        /// <summary>The SID of the Flex Plugin.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? PluginSid { get; set; }
#nullable restore
#else
        public string PluginSid { get; set; }
#endif
        /// <summary>The URL of where the Flex Plugin Version JavaScript bundle is hosted on.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? PluginUrl { get; set; }
#nullable restore
#else
        public string PluginUrl { get; set; }
#endif
        /// <summary>Whether the Flex Plugin Version is archived. The default value is false.</summary>
        public bool? PluginVersionArchived { get; set; }
        /// <summary>The SID of the Flex Plugin Version.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? PluginVersionSid { get; set; }
#nullable restore
#else
        public string PluginVersionSid { get; set; }
#endif
        /// <summary>Whether to validate the request is authorized to access the Flex Plugin Version.</summary>
        public bool? Private { get; set; }
        /// <summary>The name that uniquely identifies this Flex Plugin resource.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? UniqueName { get; set; }
#nullable restore
#else
        public string UniqueName { get; set; }
#endif
        /// <summary>The absolute URL of the Flex Plugin resource.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? Url { get; set; }
#nullable restore
#else
        public string Url { get; set; }
#endif
        /// <summary>The latest version of this Flex Plugin Version.</summary>
#if NETSTANDARD2_1_OR_GREATER || NETCOREAPP3_1_OR_GREATER
#nullable enable
        public string? Version { get; set; }
#nullable restore
#else
        public string Version { get; set; }
#endif
        /// <summary>
        /// Instantiates a new <see cref="global::Soenneker.Twilio.OpenApiClient.Models.Flex_v1_plugin_configuration_configured_plugin"/> and sets the default values.
        /// </summary>
        public Flex_v1_plugin_configuration_configured_plugin()
        {
            AdditionalData = new Dictionary<string, object>();
        }
        /// <summary>
        /// Creates a new instance of the appropriate class based on discriminator value
        /// </summary>
        /// <returns>A <see cref="global::Soenneker.Twilio.OpenApiClient.Models.Flex_v1_plugin_configuration_configured_plugin"/></returns>
        /// <param name="parseNode">The parse node to use to read the discriminator value and create the object</param>
        public static global::Soenneker.Twilio.OpenApiClient.Models.Flex_v1_plugin_configuration_configured_plugin CreateFromDiscriminatorValue(IParseNode parseNode)
        {
            if(ReferenceEquals(parseNode, null)) throw new ArgumentNullException(nameof(parseNode));
            return new global::Soenneker.Twilio.OpenApiClient.Models.Flex_v1_plugin_configuration_configured_plugin();
        }
        /// <summary>
        /// The deserialization information for the current model
        /// </summary>
        /// <returns>A IDictionary&lt;string, Action&lt;IParseNode&gt;&gt;</returns>
        public virtual IDictionary<string, Action<IParseNode>> GetFieldDeserializers()
        {
            return new Dictionary<string, Action<IParseNode>>
            {
                { "account_sid", n => { AccountSid = n.GetStringValue(); } },
                { "changelog", n => { Changelog = n.GetStringValue(); } },
                { "configuration_sid", n => { ConfigurationSid = n.GetStringValue(); } },
                { "date_created", n => { DateCreated = n.GetDateTimeOffsetValue(); } },
                { "description", n => { Description = n.GetStringValue(); } },
                { "friendly_name", n => { FriendlyName = n.GetStringValue(); } },
                { "phase", n => { Phase = n.GetIntValue(); } },
                { "plugin_archived", n => { PluginArchived = n.GetBoolValue(); } },
                { "plugin_sid", n => { PluginSid = n.GetStringValue(); } },
                { "plugin_url", n => { PluginUrl = n.GetStringValue(); } },
                { "plugin_version_archived", n => { PluginVersionArchived = n.GetBoolValue(); } },
                { "plugin_version_sid", n => { PluginVersionSid = n.GetStringValue(); } },
                { "private", n => { Private = n.GetBoolValue(); } },
                { "unique_name", n => { UniqueName = n.GetStringValue(); } },
                { "url", n => { Url = n.GetStringValue(); } },
                { "version", n => { Version = n.GetStringValue(); } },
            };
        }
        /// <summary>
        /// Serializes information the current object
        /// </summary>
        /// <param name="writer">Serialization writer to use to serialize this model</param>
        public virtual void Serialize(ISerializationWriter writer)
        {
            if(ReferenceEquals(writer, null)) throw new ArgumentNullException(nameof(writer));
            writer.WriteStringValue("account_sid", AccountSid);
            writer.WriteStringValue("changelog", Changelog);
            writer.WriteStringValue("configuration_sid", ConfigurationSid);
            writer.WriteDateTimeOffsetValue("date_created", DateCreated);
            writer.WriteStringValue("description", Description);
            writer.WriteStringValue("friendly_name", FriendlyName);
            writer.WriteIntValue("phase", Phase);
            writer.WriteBoolValue("plugin_archived", PluginArchived);
            writer.WriteStringValue("plugin_sid", PluginSid);
            writer.WriteStringValue("plugin_url", PluginUrl);
            writer.WriteBoolValue("plugin_version_archived", PluginVersionArchived);
            writer.WriteStringValue("plugin_version_sid", PluginVersionSid);
            writer.WriteBoolValue("private", Private);
            writer.WriteStringValue("unique_name", UniqueName);
            writer.WriteStringValue("url", Url);
            writer.WriteStringValue("version", Version);
            writer.WriteAdditionalData(AdditionalData);
        }
    }
}
#pragma warning restore CS0618
